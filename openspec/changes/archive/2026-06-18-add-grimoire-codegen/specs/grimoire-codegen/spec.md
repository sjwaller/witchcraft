...
### Requirement: Build does not require an external toolchain
`grimoire build` SHALL produce the executable without requiring a separately installed C/Rust compiler or system linker on the build machine; the linker needed to emit the executable SHALL be bundled. A green build SHALL remain a structural guarantee only, never an assertion that inferred values are correct.

#### Scenario: Build on a machine without a system linker installed
- **WHEN** `grimoire build` runs on a machine that has the `grimoire` binary but no separately installed compiler or linker
- **THEN** it still produces a working executable
