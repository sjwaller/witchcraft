# capability-effects Specification

## Purpose
Provide a compile-time capability/effect discipline: operations and functions may
declare required capabilities, a lexical region may grant them, and performing an
operation whose required capability is not granted is a compile-time error. This
is the single mechanism that `governed-memory` (scope) and `bounded-familiar`
(permits) specialise; it adds a structural axis of checking only — never a claim
that a permitted operation is correct or wise (§8). Capabilities are erased before
lowering and have no runtime representation.
